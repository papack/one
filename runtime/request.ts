...
function requestDetails(
  element: Element,
): { method: RequestMethod; url: string } | null {
  for (const attribute of requestAttributes) {
    const url = element.getAttribute(`data-${attribute}`);
    if (url !== null)
      return { method: attribute.toUpperCase() as RequestMethod, url };
  }
  return null;
}
...
export async function submit(form: HTMLFormElement): Promise<void> {
  ...
  if (method === "GET") {
    ...
    for (const [name, value] of formData)
      if (typeof value === "string") parsedUrl.searchParams.append(name, value);
    url = parsedUrl.href;
  } else if (form.enctype === "multipart/form-data") {
    init.body = formData;
  } else if (form.enctype === "text/plain") {
    init.body = Array.from(
      formData,
